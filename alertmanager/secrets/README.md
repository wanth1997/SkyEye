...
Alertmanager 啟動時讀取這裡的檔案作為 credential。
...
- **檔案權限 `0644`，目錄權限 `0755`**
  - Alertmanager container 內部以 `nobody` (uid 65534) 執行，無法讀取 ubuntu (uid 1000) 所有的 0600 檔案
...
- 輪替：每 180 天換一次（Telegram `/revoke` 重新拿 token）
